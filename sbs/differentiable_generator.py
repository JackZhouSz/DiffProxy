...
def get_real_generator(generator_name):
    if generator_name in sbs_lib:
        ...
        return sbs_generators.generator_lookup_table[generator_name](graph_filename, 'generator', sat_dir, 256)
    else:
...
class StyleGANCond(DifferentiableGenerator):
    ...
    def get_normalizer(self):
        params = sbs_generators.generator_lookup_table[self.generator_name].get_params()
        normalizer = sbs_generators.SBSGenerators.get_normalizer(params, batch_size=1, device=self.device)
        return normalizer

    def get_standarizer(self):
        params = sbs_generators.generator_lookup_table[self.generator_name].get_params()
        normalizer = sbs_generators.SBSGenerators.get_standarizer(params, batch_size=1, device=self.device)
        return normalizer

    def get_reguluarizer(self):
        params = sbs_generators.generator_lookup_table[self.generator_name].get_params()
        regularizer  = sbs_generators.SBSGenerators.get_regularizer(params, batch_size=1, device=self.device)
        return regularizer
